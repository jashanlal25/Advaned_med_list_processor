...
def _generate_html_inner():
    ...
    if output_format in ('old', 'both'):
        ...
        response_payload['old_file'] = _browser_result_payload(html_old)
    ...
    if output_format in ('new', 'both'):
        ...
        response_payload['new_file'] = _browser_result_payload(html_new)
...
def _browser_result_payload(html):
    """Return the generated bytes with the generation response.

    Vercel instances do not share process memory, so a later token download can
    hit another instance. Compress large HTML before including it in the same
    response; the client keeps it in memory for save, preview and sharing.
    """
    import base64 as _base64
    import gzip as _gzip
    data = html.encode('utf-8')
    if len(data) <= 512 * 1024:
        return {'encoding': 'utf8', 'data': html}
    return {'encoding': 'gzip-base64', 'data': _base64.b64encode(_gzip.compress(data, compresslevel=6)).decode('ascii')}
...
#
# Architecture: the incoming file NEVER touches Vercel /tmp or any server-side
# session. The validated bytes are base64-embedded into the chooser page HTML
# returned by this SAME request (Chrome renders the POST response directly),
# and the chooser JS commits them to browser IndexedDB. From that point the
# file lives entirely on the device — no later request depends on server state.
# ---------------------------------------------------------------------------
import base64 as _base64
...
def share_target():
    ...
    payload = {
        'filename': filename,
        'type': SHARED_FILE_TYPE_LABELS.get(ext, 'File'),
        'ext': ext,
        'size': len(file_data),
        'data_b64': b64,
    }
